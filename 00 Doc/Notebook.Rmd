...
```{SQL}
select * from GASISDATA where RESTEMP IS NOT NULL and RESTEMP > 0 and SYSNM IS NOT NULL and ERANM IS NOT NULL AND STATE IS NOT NULL
```

We created an option to exclude or include outliers, as they may skew the graph and thus our conclusions about what the graph says. We also created a list of dimensions for which we would like to use to compare temperatures, and displayed this in a drop down for the user to select. An example of this code is below:
```{r}
